...
function knowledge(s,sc,a) {
  ...
  if (id==='SC40') {
    const product=k.products[a.product_type];
    if (!product) return err('invalid_input','product_type');
    const summary={
      ogpo:['ОГПО покрывает вред здоровью и имуществу других людей при ДТП; собственный автомобиль не покрывается.','ОГПО ЖКО кезінде басқа адамдардың денсаулығы мен мүлкіне келтірілген зиянды өтейді; өз көлігіңізді қамтымайды.'],
      casco:['КАСКО Стандарт покрывает ущерб при ДТП, угон, полную гибель, стихийные бедствия, падение предметов и вандализм. Износ и механические поломки исключены.','КАСКО Стандарт ЖКО зақымын, ұрлықты, толық жойылуды, табиғи апаттарды, заттардың құлауын және вандализмді қамтиды; тозу мен механикалық ақау өтелмейді.'],
      property:['Страхование жилья покрывает пожар, затопление, кражу, стихийные бедствия и ответственность перед соседями.','Тұрғын үй сақтандыруы өрт, су басу, ұрлық, табиғи апаттар мен көршілер алдындағы жауапкершілікті қамтиды.'],
      accident:['Страхование от несчастных случаев покрывает травмы и инвалидность вследствие несчастного случая; болезнь не покрывается.','Жазатайым оқиғадан сақтандыру жарақат пен мүгедектікті қамтиды; ауру өтелмейді.'],
      travel:['При заболевании за границей сначала свяжитесь с круглосуточным медицинским ассистансом; самостоятельные расходы нужно согласовать заранее.','Шетелде ауырсаңыз, алдымен тәулік бойғы медициналық ассистансқа хабарласыңыз; шығындарды алдын ала келісу керек.'],
      dms:['Покрытие ДМС зависит от программы Базовая или Комфорт; для проверки конкретной услуги нужен полис.','ДМС қамтуы Базалық не Комфорт бағдарламасына байланысты; нақты қызметті тексеру үшін полис керек.']};
    return {answer:tr(s,...summary[a.product_type]),facts:product};
  }
...
function runAction(s,sc,name,a,execute=false) {
  ...
  switch(name) {
    ...
    case 'transfer_to_operator':{
      ...
      const r={queue,simulated:true,context:{client_id:s.client_id,scenario_id:sc.scenario_id,slots:clone(a),queued_scenarios:[...s.queued_scenarios]}};
      if(execute)s.journal.push({name,...r});return r;
    }
...
export function processTurn(s,decision,{turnId,transcript=''}={}) {
  ...
  const finish=(reply,newStatus=s.status)=>{
  ...
  if(scenarios[0].confidence<0.75){
    ...
    if(s.low_confidence_count>=2){actions.push({name:'transfer_to_operator',mode:'execute',result:{queue:'operator_general',simulated:true}});return finish(tr(s,'Не удалось уточнить запрос; в демо передаю контекст оператору.','Сұраныс анықталмады; демода мәліметті операторға беремін.'),'handoff');}
  ...
  for(const name of sc.actions){
    ...
    if(!def)return finish(errorReply(s,err('service_unavailable',name)),'handoff');
    ...
    if(def.irreversible){
      ...
      if(preview.error){
        ...
        actions.push({name:'transfer_to_operator',mode:'execute',result:{queue:sc.handoff?.queue??'operator_general',simulated:true,context:{scenario_id:chosen,slots:clone(slots)}}});
      ...
      if(result.error){
        if(chosen==='SC12'&&name==='get_policy'&&result.error.code==='not_found'){
          s.pending_confirmation=null;
          actions.push({name:'transfer_to_operator',mode:'execute',result:{queue:'operator_general',simulated:true,context:{scenario_id:chosen,slots:clone(slots)}}});
